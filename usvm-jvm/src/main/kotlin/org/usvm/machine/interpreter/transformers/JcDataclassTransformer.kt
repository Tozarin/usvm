...
object JcDataclassTransformer : JcClassExtFeature {
    ...
    fun relatedField(clazz: JcClassOrInterface, rel : RelationType) : JcField? {
        return combName(clazz, rel).let { relationFields[it] }
    }

    private fun combName(clazz : JcClassOrInterface, rel: RelationType) : String {
    ...
    fun getLambdas() : List<JcMethod> {
        ...
        classTable.relations.forEach { rel ->
            ...
            val btwTable = when (rel) {
                is RelationType.OneToMany -> rel.tableName
                is RelationType.ManyToMany -> rel.tableName
                else -> null
            }?.let { collector.tablesInfo.get(it) }
